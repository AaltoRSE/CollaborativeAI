...
from tasks.task_interface import MultiRequestTask
import asyncio
import logging
...
from tasks.task_interface import TaskClass
import json
...
class CompletionService:
    ...
    def build_model_request(
        ...
        messages.extend(
            [Message(role="user", content=element.content) for element in history]
        )
        currentMessage = Message(
            role="user", content=[TextMessage(type="text", text=currentElement.text)]
        )

        if currentElement.image:
            currentMessage.content.append(
                ImageMessage(
                    type="image_url", image_url=ImageURL(url=currentElement.image)
                )
            )
        ...
        grpc_taskRequest.request = json.dumps(
            [message.model_dump() for message in messages]
        )
        return grpc_taskRequest

    def build_model_request_from_open_AI_request(
        self, request: OpenAIBasedDataRequest
    ) -> grpc_models.taskRequest:
        for message in request.userMessages:
            if message.role == "system":
                raise ValueError(
                    "System messages are not allowed in the openAI request"
                )
    ...
    def interpret_model_response_openAI(
